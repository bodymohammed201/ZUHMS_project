...
class PatientRepository implements PatientRepositoryInterface
{
   ...
   public function update($request)
   {
       ...
       if($request->status==0 && $Patient->status == 1){
        $Patient->room->delete();
       }
       else if($request->status==1 && $Patient->status == 1){
        $Patient->room->update(['room' => $request->room]);
       }
       else if($request->status==1 && $Patient->status ==0 ){
               Room::create([
            'room'=>$request->room,
